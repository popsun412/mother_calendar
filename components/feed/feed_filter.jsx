            ...
            <div className='mb-7 mx-3.5'>
                ...
                <div className='flex flex-wrap'>
                    {["국어", "영어", "수학", "과학", "사회", "미술", "음악", "체육", "생활", "기타", "부모"].map((_subject, index) => {
                        const _checkIndex = filterParams.subjects.findIndex((_item) => _item == _subject);
                        return (
                            <div className={`py-2 px-2 mr-3 mb-3 rounded border text-center ${_checkIndex >= 0 ? "border-orange4 textOrange4" : "border-gray4 textGray4"}`} key={index} onClick={() => {
                                if (_checkIndex >= 0) {
                                    filterParams.subjects.splice(_checkIndex, 1);
                                } else {
                                    filterParams.subjects.push(_subject);
                                }

                                setFilterParams({ ...filterParams, subjects: [].concat(filterParams.subjects) });
                            }}>
                                <span>{_subject}</span>
                            </div>
                        )
            ...
            <div className='mb-7 mx-3.5'>
                ...
                <div className='flex justify-between space-x-2'>
                    {["체험", "엄마표 교육", "사교육"].map((_interest, index) => {
                        const _checkIndex = filterParams.interests.findIndex((_item) => _item == _interest);

                        return (
                            <div className={`flex-auto py-2 px-3 rounded border text-center ${_checkIndex >= 0 ? "border-orange4 textOrange4" : "border-gray4 textGray4"}`} key={index} onClick={() => {
                                if (_checkIndex >= 0) {
                                    filterParams.interests.splice(_checkIndex, 1);
                                } else {
                                    filterParams.interests.push(_interest);
                                }

                                setFilterParams({ ...filterParams, interests: [].concat(filterParams.interests) });
                            }}>
                                <span>{_interest}</span>
                            </div>
                        )
