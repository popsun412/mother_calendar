...
const Login = () => {
    ...
    const login = () => {
        if (!validateEmail(email)) return alert("이메일을 확인해주세요.");
        if (password.trim().length == 0) return alert("비밀번호를 확인해주세요.");
                ...
                <div className="px-6">
                    ...
                    <div className="flex items-center justify-center rounded-md bg-gray2 px-4 py-4">
                        <input
                            ...
                            onKeyDown={(e) => {
                                if (e.key == 'Enter' || e.key == 'enter') login();
                            }}
                        />
                        <svg className={`w-6 h-6 ${showPassword ? "" : "textGray4"} `} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" onClick={() => setShowPassword(!showPassword)}>
                            <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                            <path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd"></path>
                        </svg>
                    </div>
                    <div className="text-xs font-normal textGray3 mt-3.5 flex items-center justify-center underline underline-offset-2" onClick={() => router.push('/resendpw')}>비밀번호를 잊으셨나요?</div>
