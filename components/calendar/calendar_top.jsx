...
export default function CalendarTop(props) {
  ...
  return (
    <div className="flex px-5 pt-4 pb-3 space-x-4 border-b border-color border-[#e0e0e0] overflow-x-auto scrollbar-hide">
      <CalendarTopAvatar
      ...
      {(props.userInfo.friends ?? []).map((_friend) => (
        <span
          className={`flex-shrink-0 rounded-full w-9 h-9 ${props.selectedUserUid == _friend.uid ? "ring-[3px] ring-[#FF6035]" : "ring-1 ring-[#e0e0e0]"}`}
          key={_friend.uid}
          onClick={() => router.push(`/calendar?friend=${_friend.uid}`)}
        >
          <div
      ...
      <Link href={"/neighbor"}>
        <button className="flex-shrink-0 rounded-full w-9 h-9 border-dashed border-gary3 border flex items-center justify-center">
