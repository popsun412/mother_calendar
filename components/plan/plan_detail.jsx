    ...
    <>
      <div className="px-5">
        ...
        {props.plan.commonPlanUid ? (
          <div
            className="border rounded-md border-color2 flex justify-between p-3 mb-4"
            onClick={() => {
        ...
        <div className="pl-3 flex flex-row space-x-8 mb-9">
          <div>
            ...
            <span className="textGray1 text-base font-normal table-row">
              {calcPercent(props.plan)}
        ...
        {auth.currentUser.uid == props.plan.createUserUid ? (
          <div className="fixed max-w-500 flex items-center justify-center left-0 right-0 bottom-6">
            {status() == 0 ? (
              <span
                className="px-5 py-3 bg5 text-base text-white font-medium rounded-full"
                onClick={() => {
                  setLockers([]);
            ...
            {status() == 1 ? (
              <span className="px-5 py-3 bg-gray4 text-base text-white font-medium rounded-full fixed bottom-6 max-w-fit">오늘 인증을 완료했어요!</span>
            ) : (
              <></>
            )}
            {status() == 2 ? (
              <Link href={`/plan/edit?planUid=${props.plan.planUid}`} passHref>
                <span className="px-5 py-3 bg5 text-base text-white font-medium rounded-full fixed bottom-6 max-w-fit">종료 계획 재시작하기</span>
              </Link>
            ) : (
              <></>
            )}
