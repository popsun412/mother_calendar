...
export default function Edit(props) {
  ...
  const getPlan = async () => {
    ...
    if (_result.data.auths.length > 0) {
      _result.data.auths.sort((a, b) => moment(a.authDt).unix() - moment(b.authDt).unix());
      maxDate.current = moment(_result.data.auths[_result.data.auths.length - 1].authDt).toDate();
    }
  };
  ...
  const onSave = async () => {
    ...
    // 인증날짜 확인
    if (_updateParam.endDate <= maxDate.current) _updateParam.endDate = maxDate.current;
  ...
  const getMaxDate = () => {
    ...
    const _endDate = planInfo.endDate == null ? null : moment(planInfo.endDate).subtract(1, "d").toDate();
        ...
        <div className="">
          <div className="relative flex py-4">
            ...
            <div className="flex flex-auto justify-between items-center z-50 px-5">
              ...
              <span className={`text-base font-medium ${isActive() ? "textOrange5" : "textGray4"}`} value="false" onClick={onSave}>
                완료
              </span>
            </div>
          </div>
                    ...
                    >
                      <div className="flex-auto border border-gary3 rounded-md text-sm textGray2 text-center py-1 flex items-center justify-center">
                        <span className="text-xs font-medium pl-2">
                          {planInfo.endDate == null ? "종료날짜" : moment(planInfo.endDate).format("YYYY년 M월 D일")}
                        </span>
                        <ChevronRight className="rotate-90" />
                      </div>
                    </CustomMobileDatepicker>
            ...
            <div className="mb-8 flex flex-col">
              ...
              {openTime ? (
                <div className="flex space-x-1.5 mt-3">
                  <CustomTimepicker onChange={(time) => setPlanInfo({ ...planInfo, startTime: time })} value={planInfo.startTime}>
                    <div className="flex-auto border border-gary3 rounded-md text-sm textGray2 text-center py-1 flex items-center justify-center">
                  ...
                  <CustomTimepicker onChange={(time) => setPlanInfo({ ...planInfo, endTime: time })} value={planInfo.endTime}>
                    <div className="flex-auto border border-gary3 rounded-md text-sm textGray2 text-center py-1 flex items-center justify-center">
