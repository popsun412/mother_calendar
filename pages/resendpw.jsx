      ...
      <div className="flex flex-col w-full h-screen overflow-y-auto">
        ...
        {sendEmailStatus ? (
          <div className="mx-5 my-14">
            <pre className="max-w-full whitespace-pre-wrap break-all text-xl" style={{ fontFamily: "Bazzi" }}>
              {email}로 이메일이 발송되었습니다.
            </pre>
            <pre className="max-w-full whitespace-pre-wrap break-all mb-4 text-xl" style={{ fontFamily: "Bazzi" }}>
              이메일 내 링크를 클릭해 비밀번호를 재설정 후 재로그인해주세요.
            </pre>
            <p className="text-sm">이메일을 수신하지 못했다면 스팸함 또는 해당 이메일 서비스의 설정을 확인해주세요.</p>
          </div>
        ) : (
          <div className="mx-5 my-14">
            <pre className="max-w-full whitespace-pre-wrap break-all mb-4 text-xl text-left" style={{ fontFamily: "Bazzi" }}>
              가입하신 이메일을 입력해주세요.
            </pre>
            <input
              type="text"
        ...
        {sendEmailStatus ? (
          ...
          <div
            className={`rounded h-12 mx-5 mb-6 flex justify-center items-center text-white text-sm font-semibold hover:cursor-pointer ${
              validateEmail(email) ? "bg5" : "bg-gray4"
            }`}
            onClick={sendEmail}
          >
            <span>다음</span>
