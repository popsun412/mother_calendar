...
import ExMapBody from "../components/exmap/exmap_body";
    ...
    return (
        <>
            <header className="fixed max-w-500 top-0 left-0 right-0 visible opacity-100 bg-white z-100" style={{ marginBottom: "-50px" }}>
                <div
                    className="my-auto mx-auto py-0 px-4 relative flex items-center w-full bg-white border-b border-solid border-gray3"
                    style={{ height: "50px" }}
                >
                    <div className="flex-1 flex items-center justify-between">
                        <div className="absolute left-0 right-0 mx-20 text-center" style={{ letterSpacing: "-0.3px" }}>
                            체험
                        </div>
                        <img src="/images/ic_back.png" className="w-10 relative -left-4 flex-shrink-0" onClick={() => window.history.back()} />
                        <div className="flex space-x-2">
                            <img src={`${isList ? "/images/ic-map.png" : "/images/ic_list.png"}`} style={{ width: 18 }} onClick={() => setIsList(!isList)} />
            ...
            {isList ? (
                                        ...
                                        <Link href={{ pathname: "/item", query: { commonItemUid: item.commonItemUid } }} key={idx} passHref>
                                            <div className="flex mb-5">
                                                <div className="mr-4 block relative">
                                                    ...
                                                    <img
                                                        src={`/images/ic_${item.bookmark ? "bookmarked.png" : "bookmark.png"}`}
                                                        className="block absolute bottom-0 right-0 mr-2 mb-1.5 w-3"
                                                        onClick={() => addBookmark(item.commonItemUid, idx)}
                                                    />
                                                </div>
                                                <div>
                ...
                <ExMapBody items={items} param={param} />
            )}
