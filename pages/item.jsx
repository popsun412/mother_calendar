...
import CommonItemSlider from "../components/common_item/common_item_slider";
    ...
    <>
      <div style={{ fontFamily: "SuncheonR" }}>
        <header className="sticky top-0 left-0 right-0 opacity-100 visible z-100" style={{ marginBottom: "-50px" }}>
          ...
          >
            {scrollPosition > 60 ? (
              <div className="flex mx-4 w-full items-center relative">
                <div className="absolute flex justify-center items-center mx-3 left-0 right-0">
                  <img
                    className="w-4 h-4 mr-2"
                    src={getSubjectImage(data.subject)}
                    onClick={() => {
                      window.history.back();
                    }}
                  />
        ...
        <main className="mb-28">
          <section className="mb-7">
            <div className="block relative">
              ...
              <span className="block absolute bottom-0 left-0 text-lg text-white font-bold ml-5 mb-11" style={{}}>
                {data.name}
              </span>
              ...
              {data.subject != "부모" ? (
                <div className="block absolute bottom-0 right-0 mr-5 mb-5">
                  <img
                    src={`/images/ic_${data.bookmark ? "bookmarked.png" : "bookmark.png"}`}
          ...
          <section className="mx-5 mb-8">
            <div className="text-base font-semibold mb-3" style={{ letterSpacing: "-0.32px", fontFamily: "SuncheonR" }}>
              어떤 아이템인가요?
            </div>
            {data.images != null ? <CommonItemSlider images={data.images} /> : <></>}
            <div className="text-sm" style={{ letterSpacing: "-0.28px", lineHeight: "21px" }}>
              <pre className="max-w-full whitespace-pre-wrap break-all" style={{ fontFamily: "SuncheonR" }}>
                {data.description}
              </pre>
            </div>
          </section>
          {recommData.length > 0 ? (
            <section className="mx-5">
              <div className="text-base font-semibold mb-4" style={{ letterSpacing: "-0.32px" }}>
                추천 계획
              </div>
              <div>
                {recommData.map((item, idx) => {
                  return (
                    <Link key={item.commonPlanUid} href={`/plandetail?commonPlanUid=${item.commonPlanUid}`} passHref>
                      <div
                        className="flex py-4 pr-4 text-sm mb-4 justify-between items-center"
                      ...
                      >
                        ...
                        <img
                          className="w-5"
                          src="/images/ic_check_circle.png"
                          onClick={(e) => {
                            e.preventDefault();
        ...
        {data.subject != "부모" ? (
          <aside className="fixed max-w-500 bottom-0 left-0 right-0 z-100">
            <div className="relative mx-auto my-0 bg-white">
              ...
              >
                <span
                  className={`text-sm text-white text-center p-4 m-5 w-full rounded-md ${!data.bookmark ? "bg5" : "bg-gray4"}`}
