...
const Account = () => {
  ...
  const list = (anchor) => (
    <Box role="presentation" onKeyDown={(e) => setBottomSheetOpen(false)}>
      <GlobalStyles
        styles={{
          ".MuiPaper-root": {
      ...
      <div className="mx-5 my-6">
        ...
        {userInfo?.isShare ?? false ? (
          <div>
            ...
            <div
              className="w-full h-12 bg5 leading-12 text-center text-white rounded-md"
              onClick={() => {
      ...
      <header className="sticky top-0 left-0 bottom-0 visible opacity-100 bg-white z-100" style={{ marginBottom: "-50px" }}>
        <div className="my-auto mx-auto py-0 px-4 relative flex items-center w-full bg-white border-b border-solid" style={{ height: "50px" }}>
          <div className="flex-1 flex items-center relative">
            ...
            <div className="absolute left-0 right-0 mx-10 text-center text-base" style={{ letterSpacing: "-0.3px" }}>
              계정 관리
            </div>
          </div>
        </div>
      ...
      <main style={{ marginTop: "50px" }}>
        <section className="pt-6">
          <div className="flex mx-5 mb-4 justify-between" style={{ height: "50px" }}>
            ...
            <div className="text-sm textGray2" style={{ lineHeight: "50px" }}>
              {auth.currentUser?.email ?? ""}
            </div>
          </div>
          <div className="flex mx-5 my-4 justify-between" style={{ height: "50px" }}>
            <div style={{ fontSize: "15px", lineHeight: "50px" }}>비밀번호</div>
            <div className="text-sm textOrange5" style={{ lineHeight: "50px" }} onClick={() => router.push("/resendpw")}>
              변경하기
            </div>
          </div>
          <div className="flex items-center mx-5 my-4 justify-between" style={{ height: "50px" }}>
            ...
            <Switch checked={userInfo?.isShare ?? false} onChange={(e) => setBottomSheetOpen(true)} offColor="#e0e0e0" onColor="#3C81E1" />
          </div>
          <div className="mx-5 my-4 justify-between">
            ...
            <div className="ml-9" style={{ marginTop: "-10px" }}>
              <Checkbox
                checked={userInfo.emailAgree}
                onChange={() => setUserInfo({ ...userInfo, emailAgree: !userInfo.emailAgree })}
                style={{ color: "#FF6035" }}
              />
              <span className="textGray3 text-xs font-medium">이메일</span>
              <Checkbox checked={userInfo.smsAgree} onChange={() => setUserInfo({ ...userInfo, smsAgree: !userInfo.smsAgree })} style={{ color: "#FF6035" }} />
              <span className="textGray3 text-xs font-medium">SMS</span>
              <Checkbox
                checked={userInfo.postAgree}
                onChange={() => setUserInfo({ ...userInfo, postAgree: !userInfo.postAgree })}
                style={{ color: "#FF6035" }}
              />
              <span className="textGray3 text-xs font-medium">우편</span>
            </div>
          </div>
